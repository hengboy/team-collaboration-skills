...
**更新日期**: 2026-03-19  
**分析对象**: AI 协作团队 Skills 需求流转链路（含 Feature Coordinator、frontend-design）
**版本**: v7.0.0 - 混合流程 + 联合评审
...
```
            ...
            ┌──────┴──────┐
            ↓             ↓
   [Project Manager]  [Feature Coordinator]
      ↓ 项目计划      ↓ 组织并行工作
            │      ┌───┴───┐
            │      ↓       ↓
...
| 3 | Feature Coordinator | - | 组织联合评审 | `.collaboration/features/*/review.md` | Frontend-Design + Tech Lead |
...
- **阶段 3 联合评审**: Feature Coordinator 组织，自动检测 4 个维度冲突，最多 5 轮修改
...
- **Feature Coordinator** → 组织并行工作、联合评审、冲突检测
...
**注意**：
- `feature-name` **不是由 Feature Coordinator 确认**
- `feature-name` 由 **Product Manager 在创建 PRD 时确定**
- Feature Coordinator 在评审主链路中负责**验证和确保一致性**
...
Product Manager → 创建 .collaboration/features/{feature-name}/prd.md
                ↓
Feature Coordinator → 从 PRD 路径提取 {feature-name}
                ↓
...
### 阶段 3 联合评审：Feature Coordinator

**Frontend-Design** 和 **Tech Lead** 都完成后，由 **Feature Coordinator** 组织联合评审。
...
│   │   │   └── review.md               # Feature Coordinator 输出（评审记录）
│   │   └── payment-feature/
│   │       ├── prd.md
│   │       ├── design.md
...
| feature-name 一致性 | 可能不一致 | Feature Coordinator 自动检查和修正 |
| 冲突检测 | 需要准确识别 | 4 个维度检测规则需要持续优化 |
| Subagent 间通信 | 需要中转 | 通过 Feature Coordinator 转发消息 |
...
2. **Frontend-Design + Tech Lead → 联合评审**: Feature Coordinator 组织，自动冲突检测
...
1. **Feature Coordinator** - 组织并行工作、联合评审、冲突检测
...
1. **使用 Feature Coordinator** - 复杂功能使用联合评审机制
...
| feature-coordinator | `skills/feature-coordinator/SKILL.md` |
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v7.0.0 | 2026-03-19 | 新增 Feature Coordinator、联合评审机制、4 维度冲突检测 |
