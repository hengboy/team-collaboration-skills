...
8. 回收当前仓或业务仓的实现证据
9. 推动 `qa-engineer`、`code-reviewer` 和 `git-commit` 完成统一收口
...
- `single-repo` 下，当前仓实现角色消费 handoff，并回传 diff、变更文件路径、测试结果或构建结果
...
- `single-repo` 下，当前仓 `frontend` / `backend-*` 消费 handoff 并实现修复；回收到 diff、变更文件路径、测试结果或构建结果后，`bug-coordinator` 才能进入 `qa-engineer`、`code-reviewer` 和 `git-commit`。
...
- [ ] `single-repo` 下已回收当前仓实现证据，或 `split-repo` 下已回收业务仓 PR / 测试结果 / 变更摘要后再以 Bug 模式进入 QA / Review
...
1. `single-repo`：当前仓实现角色消费 `.collaboration/bugs/{bug-name}/frontend-handoff.md` / `.collaboration/bugs/{bug-name}/backend-handoff.md` 并实现修复，再把本仓实现证据交回 `bug-coordinator`
2. `split-repo`：外部业务仓消费 handoff 并实现修复，再回传 PR、测试结果和变更摘要
3. `bug-coordinator` 回收实现证据后统一进入 `qa-engineer`
4. QA 完成后进入 `code-reviewer`，阻塞问题关闭后进入 `git-commit`
