...
- 标准需求流转中，`frontend-design` 应由 `feature-coordinator` 以 subagent 方式调用，并与 `project-manager`、`tech-lead` 首轮并行启动
- 设计产物先回流到协调器，与计划和技术产物一起参加联合评审，通过后再进入前端实现
