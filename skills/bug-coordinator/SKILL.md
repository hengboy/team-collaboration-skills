...
- 信息不足时先补齐上下文，再进入下游判责或修复；如需向用户补充提问，必须使用选择框并允许填写自定义意见
...
- 若发现“修复”实质上是新增功能、额外接口、额外页面或新增验收范围，必须暂停当前缺陷链路；若需要向用户确认，使用选择框说明是否回退到 `product-manager` 重头开始。
...
- [ ] 如本轮存在面向用户的补充提问或回退确认，均已使用选择框并允许填写补充意见
