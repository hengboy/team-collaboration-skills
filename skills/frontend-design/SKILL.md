...
- 作为 `feature-coordinator` 的 subagent 运行时，可直接基于 `.collaboration/features/{feature-name}/prd.md` 启动，并可与 `project-manager`、`tech-lead` 首轮并行执行；`.collaboration/features/{feature-name}/tech.md`、`.collaboration/features/{feature-name}/api.yaml` 后续补齐后用于校准或修订。
- 阶段性结果和修订结果先回传给 `feature-coordinator`，由协调器统一向用户询问“通过”还是“继续澄清/修订”。
- 若修订请求引入超出当前 PRD 的新增功能或新增页面范围，则停止当前设计修订，回传 `feature-coordinator` 并要求重启到 `product-manager`。
...
标准需求流转中，`frontend-design` 应由 `feature-coordinator` 以 subagent 方式调用，并与 `project-manager`、`tech-lead` 首轮并行启动；设计产物先回流到协调器参加联合评审，通过后再进入前端实现。

1. `feature-coordinator` 可与 `project-manager`、`tech-lead` 并行调用 `frontend-design`，并监听设计产物与修订回合
2. 首轮面向用户的正式评审前，`feature-coordinator` 必须先回收 `plan.md`、`tech.md`、`api.yaml`、`design.md`、`design-components.md`
3. `feature-coordinator` 汇总设计、技术、计划与 API 产物进行评审，并向用户询问“通过”还是“继续澄清/修订”
4. 若评审涉及设计与技术冲突，`feature-coordinator` 可并行回派给 `frontend-design` 与 `tech-lead`
...
- 标准链路：`feature-coordinator` -> `frontend-design` subagent -> `feature-coordinator` -> `frontend`
