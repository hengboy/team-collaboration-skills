...
标准需求流转中，`frontend-design` 应由 `master-coordinator` 以 subagent 方式调用，设计产物先回流到协调器参加联合评审，通过后再进入前端实现。

1. `master-coordinator` 以 subagent 方式调用 `frontend-design`，并监听设计产物与修订回合
2. `master-coordinator` 汇总 `.collaboration/features/{feature-name}/design.md`、`.collaboration/features/{feature-name}/design-components.md` 与技术方案进行评审
3. 每轮结果先由协调器向用户询问“通过”还是“继续澄清/修订”
4. 联合评审通过后，`frontend` 消费设计产物与 `.collaboration/features/{feature-name}/api.yaml` 进入实现
5. 实现完成后再进入 `qa-engineer`
