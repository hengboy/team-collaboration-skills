...
标准需求流转中，`project-manager` 通常由 `master-coordinator` 以 subagent 方式调用，并可与 `tech-lead`、`frontend-design` 并行执行；产物回流后由协调器与技术、设计结果一起进入首轮联合评审准备。

1. `project-manager` 基于 `.collaboration/features/{feature-name}/prd.md` 生成 `.collaboration/features/{feature-name}/plan.md`
2. 当前主会话保持在 `master-coordinator`，`plan.md` 回传时 `tech-lead` 与 `frontend-design` 可以已在并行推进
3. `master-coordinator` 在计划、技术、API 与设计产物齐备后，汇总首轮结果并向用户询问“通过”还是“继续澄清/修订”
4. 若评审意见涉及排期、资源或阶段拆分，`master-coordinator` 将修订任务回派给 `project-manager`
5. 联合评审通过后，产物再进入开发阶段
