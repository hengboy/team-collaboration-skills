...
- 审查阶段不替代实现角色直接修改代码。
- Feature 模式：
  - 重点检查实现是否符合 `tech.md`、测试资产和 QA 结论。
- Bug 模式：
...
- Feature 模式：`.collaboration/features/{feature-name}/code-review.md`
- Feature 模式：`.collaboration/features/{feature-name}/security-review.md`
- Bug 模式：`.collaboration/bugs/{bug-name}/code-review.md`
- Bug 模式：`.collaboration/bugs/{bug-name}/security-review.md`
...
- Findings 按严重程度排序
- 说明位置、影响、触发条件和建议动作
- Bug 模式重点检查根因闭环、回归保护和 handoff 边界
- 缺少上下文文档时不得输出到错误目录
- 不在审查阶段直接代改代码
