...
**审查报告必须保存到 `.collaboration/features/{feature-name}/` 目录**：

```
.collaboration/features/{feature-name}/
├── code-review.md            # 代码审查报告（必须）
└── security-review.md        # 安全审查报告（可选）
```

**重要说明**：
- `{feature-name}` 是动态的需求特性目录名称
- `feature-name` 从输入的代码路径或 PRD 路径提取
- 使用小写 kebab-case 格式
- **严禁输出到当前目录或其他位置**
...
@.collaboration/shared/coding-standards.md
