...
- Feature 模式：
  ...
  - `.collaboration/features/{feature-name}/design.md` 与 `.collaboration/features/{feature-name}/design-components.md` 必须包含 YAML frontmatter，至少写入 `feature: {feature-name}` 与当前生效的 `workspace_mode`；修订时沿用原 frontmatter 并按当前上下文更新。
...
- [ ] Feature 模式下：`design.md` 与 `design-components.md` 的 frontmatter 已写入 `feature` 与 `workspace_mode`
