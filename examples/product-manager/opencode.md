...
默认在业务仓中使用。调用 `product-manager` 后，先用选择框确认工作空间模式：若 `.collaboration/shared/workspace.md` 已存在，先确认是否沿用当前值；若文件不存在，则先确认 `workspace_mode`（只允许 `single-repo` / `split-repo`），确认后创建该文件。
...
# - 若 `.collaboration/shared/workspace.md` 已存在，先用选择框问：
#   - [ ] 继续沿用当前 workspace_mode: single-repo
#   - [ ] 改为 single-repo
#   - [ ] 改为 split-repo
#   补充意见：____
# - 若文件不存在，先用选择框问要使用的 workspace_mode（仅允许 `single-repo` / `split-repo`）：
#   - [ ] single-repo
#   - [ ] split-repo
#   补充意见：____
...
- [ ] 继续澄清/修订
补充意见：____
如果 `workspace_mode` 是 `split-repo`，联合评审通过后只用选择框提示我是否提交并推送当前协作文档，不进入 `frontend` / `backend-*`：
- [ ] 提交并推送当前协作文档
- [ ] 暂不提交，先保留当前协作文档变更
补充意见：____
...
- 每轮都先回到 `feature-coordinator`，由协调器用选择框询问用户“通过”还是“继续澄清/修订”，并允许填写补充意见
