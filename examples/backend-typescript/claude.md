...
如果 `workspace_mode` 是 `single-repo`，先确保 `./backend/` 存在；若不存在就创建，并且后续只允许在该目录内识别 TypeScript 后端源码路径与测试路径。
如果 `workspace_mode` 是 `split-repo`，确认当前仓是目标业务仓，而不是协作仓。
先根据输入路径或 frontmatter 确认唯一 feature-name。
...
如果 `workspace_mode` 是 `single-repo`，先确保 `./backend/` 存在；若不存在就创建，并且后续只允许在该目录内识别 TypeScript 后端源码路径与测试路径。
如果 `workspace_mode` 是 `split-repo`，确认当前仓是目标业务仓，而不是协作仓。
先根据输入路径或 frontmatter 确认唯一 bug-name。
